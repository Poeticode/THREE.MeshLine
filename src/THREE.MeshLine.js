import { BufferAttribute, BufferGeometry, Color, DoubleSide, FrontSide, Geometry, Matrix4, LineSegments, Mesh, ShaderChunk, ShaderMaterial, Sphere, Ray, UniformsLib, Vector2, Vector3 } from 'three-full';

export function CreateMeshLine(geometry, camera) {
	...
	MeshLine.prototype.setGeometry = function (g, c) {
		...
		if (g instanceof BufferGeometry) {
			...
			for (var j = 0; j < g.attributes.position.array.length; j += 3) {
				...
				this.positions.push(g.attributes.position.array[j], g.attributes.position.array[j + 1], g.attributes.position.array[j + 2]);
				this.positions.push(g.attributes.position.array[j], g.attributes.position.array[j + 1], g.attributes.position.array[j + 2]);
				this.counters.push(c);
				this.counters.push(c);
			}
	...
	var lineGeo = new MeshLine();
	lineGeo.setGeometry(geometry); //Create your own required geometry
	...
	return new Mesh(lineGeo.geometry, lineMat);
}
